<template>
  <div class="margin relative f15 pB30 pT30 B00 bai pL20 pR20 search_jobZ" style="width:1180px;">
    <div class="relative mB30">
     ...
     <div class="mB30">
      ...
      <span v-for="(item,index) in jobs">
        <span v-for="(item1,key1,index1) in item" class="mR15 mB10 f12 cursor" :class="{c4cafb6:jobwai==key1}" @click="tojob2(key1,index,index1)">{{key1}}</span>
      </span>
      <p class="pL82 mT20">
        <span v-for="(item2,key2,index2) in job2" :class="{c4cafb6:jobnei==key2}" @click="tojob3(item2,key2,index2)" class="mR15 mB10 f12 cursor">{{item2}}</span>
        </p>
    </div>
    ...
    <select v-model="areaName4" class="outline0 f15" v-if="area4.length!=0" id="sel4" @change="areas4">
              <option value="">请选择</option>
